...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* Cross-doc nav: link the SRS page to the published docs (Setup, Changelog…). */
    .site-nav { display: flex; flex-direction: column; gap: 3px; margin: 0 0 18px; padding-bottom: 14px; border-bottom: 1px solid var(--color-hairline); }
    .site-nav a { font-size: 12.5px; font-weight: 600; color: var(--color-muted); text-decoration: none; padding: 6px 10px; border-radius: 6px; display: flex; align-items: center; gap: 7px; }
    .site-nav a:hover { background: var(--color-surface); color: var(--color-ink); }
    .site-nav a.active { background: var(--color-primary); color: var(--color-ink); }
...
<body>
  <div class="layout">
    <aside>
      ...
      <nav class="site-nav">
        <a href="srs-index.html" class="active">📋 SRS</a>
        <a href="docs/guides/setup/">🛠️ Setup guide</a>
