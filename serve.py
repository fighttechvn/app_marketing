...
# iOS has no adb-equivalent: WDA is the standard input-injection agent. We
# forward WDA's HTTP (8100) + MJPEG (9100) ports off-device with `iproxy`
# (libimobiledevice), proxy them through here, and translate gestures to WDA's
# HTTP API. Requires WDA already RUNNING on the device (Xcode or go-ios launch).
# NOTE: WDA gesture coords are logical POINTS (window/size), not screenshot
# pixels — the frontend maps clicks via the points size reported by /status.
# ============================================================================
IPROXY = _find_bin("iproxy", "IPROXY_BIN")
IDEVICE_ID = _find_bin("idevice_id", "IDEVICE_ID_BIN")
IDEVICEINFO = _find_bin("ideviceinfo", "IDEVICEINFO_BIN")
...
def _run(cmd, timeout=20, binary=False):
    p = subprocess.run(cmd, capture_output=True, timeout=timeout)
    out = p.stdout if binary else p.stdout.decode("utf-8", "replace")
    return out, p.stderr.decode("utf-8", "replace"), p.returncode
...
def _ios_list():
    if not _have(IDEVICE_ID):
        return []
    out, _, _ = _run([IDEVICE_ID, "-l"], timeout=10)
    return [u.strip() for u in out.splitlines() if u.strip()]

def _ios_info(udid, key):
    if not udid or not _have(IDEVICEINFO):
        return ""
    out, _, rc = _run([IDEVICEINFO, "-u", udid, "-k", key], timeout=8)
...
def ios_devices():
    return {"ok": True, "iproxy": _have(IPROXY),
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def _proxy_mjpeg(self, udid):
        ...
        try:
            _ios_tunnel(udid)
            up = urllib.request.urlopen(WDA_MJPEG + "/", timeout=10)
        except Exception as e:
    ...
    def do_GET(self):
        ...
        if route == "/api/wda/devices":
            try: self._json(200, ios_devices())
            except Exception as e: self._json(500, {"ok": False, "error": str(e)})
            return
        if route == "/api/wda/status":
            try:
                q = urllib.parse.parse_qs(urllib.parse.urlparse(self.path).query)
                self._json(200, ios_status((q.get("udid") or [None])[0]))
            except Exception as e: self._json(500, {"ok": False, "error": str(e)})
            return
        if route == "/api/wda/screen":
            try:
                q = urllib.parse.parse_qs(urllib.parse.urlparse(self.path).query)
                self._bin(200, ios_screen((q.get("udid") or [None])[0]), "image/png")
            except Exception as e: self._json(502, {"ok": False, "error": str(e)})
            return
        if route == "/api/wda/mjpeg":
            q = urllib.parse.parse_qs(urllib.parse.urlparse(self.path).query)
            self._proxy_mjpeg((q.get("udid") or [None])[0])
            return
        return super().do_GET()
    def do_POST(self):
        route = self.path.split("?")[0]
